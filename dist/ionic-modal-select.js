...
/***/ (function(module, exports, __webpack_require__) {
	...
	modalSelect.$inject = ["$ionicModal", "modalSelect", "$timeout", "$filter", "$parse", "$templateCache"];
	...
	angular.module('ionic-modal-select', []).provider('modalSelect', function () {
	...
	}).directive('compile', compile).directive('modalSelect', modalSelect);
		...
		return {
			...
			link: function link(scope, iElement, iAttrs, ngModelController, transclude) {
				...
				if (iAttrs.searchTemplate) {
					...
					$ionicModal.fromTemplateUrl(multiple ? modalSelect.modalTemplateMultipleUrl : modalSelect.modalTemplateUrl, { scope: scope }).then(function (modal) {
						scope.modal = modal;
					});
				} else {
